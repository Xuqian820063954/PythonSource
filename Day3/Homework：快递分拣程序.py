...
province_list={}#存储结果字典，记录按照(省份:快递记录列表)进行存储
for record in data_list:
    #获取记录的key值
    if record[1][:2]=='新疆' or record[1][:2]=='宁夏':#新疆、宁夏
        str=record[1][:2]
    elif record[1][:4]=='黑龙江省':                   #黑龙江省
        str=record[1][:4]
    else:                                           #其他省份
        str=record[1][:3]
    if str not in province_list:                    #字典里没有该省份
        province_list[str]=[]                       #新建省份
        province_list[str].append(record)           #将快递记录添加到该省份
    else:
        province_list[str].append(record)           #若该省份已存在，则直接添加
#结果输出
print("{")
for province in province_list:
    print("    \""+province+"\":[")
    for record in province_list[province]:
        print("        ",end="")
        print(record)
    print("    ],")
print("}")
